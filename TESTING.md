...
|---|---|---|---|
| 0 | Smoke — does it run? | ~15 s | `cargo build --release && ./target/release/interflect render scenes/product.rad -o out.png` |
...
PowerShell use `.\target\release\interflect.exe` and `Get-FileHash` for `md5sum`.
...
```bash
./target/release/interflect render scenes/stress.rad -o b.png --mode normals | grep shade
./target/release/interflect render scenes/stress.rad -o l.png --mode normals --no-bvh | grep shade
...
./target/release/interflect render /tmp/bad.rad -o /tmp/b.png; echo "exit: $?"
```
...
```bash
./target/release/interflect render scenes/cornell.rad -o n.png --mode normals   # geometry & normals
...
./target/release/interflect render scenes/cornell.rad -o i.png --mode indirect  # GI in isolation
```
